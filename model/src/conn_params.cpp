 ...
 *   Modified: 2025-10-30 - Added DATAFILE support
 *                                                                         *
...
void ConnectionParams::Deserialize(const wxString& configStr) {
  ...
  if (prms.Count() >= 26) {
    ReplaySpeed = wxAtoi(prms[25]);
  }
  if (prms.Count() >= 27) {
    ReplayLoop = !!wxAtoi(prms[26]);
  }
}
...
wxString ConnectionParams::Serialize() const {
  ...
  wxString ret = wxString::Format(
      "%d;%d;%s;%d;%d;%s;%d;%d;%d;%d;%s;%d;%s;%d;%d;%d;%d;%d;%s;%d;%s;%d;%d;%s;%s;%d;%d",
      ...
      AuthToken.c_str(), FilePath.c_str(), ReplaySpeed, ReplayLoop);
...
wxString ConnectionParams::GetDSPort() const {
  if (Type == SERIAL)
    ...
    return wxString::Format("FileReplay:%s", FilePath.c_str());
  } else
    return "";
}
...
bool ConnectionParams::GetValidPort() const {
  if (Type == SERIAL && Port == "")
    ...
    return false;
  else if (Type == DATAFILE && FilePath == "")
    return false;
  else
    return true;
}
...
std::string ConnectionParams::GetStrippedDSPort() const {
  if (Type == SERIAL) {
    ...
    return ("FileReplay:" + FilePath).ToStdString();
...
NavAddr::Bus ConnectionParams::GetCommProtocol() const {
  if (Type == DATAFILE) {
    return NavAddr::Bus::N0183;  // File replay is NMEA0183
  }
