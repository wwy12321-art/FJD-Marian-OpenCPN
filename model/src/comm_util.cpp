 ...
 *   Modified: 2025-10-30 - Added detailed logging
 *                                                                         *
...
void UpdateDatastreams() {
  ...
  wxLogMessage("UpdateDatastreams: Processing %zu connections", TheConnectionParams().size());

  for (auto* cp : TheConnectionParams()) {
    wxLogMessage("  Connection: Type=%d, Enabled=%d, IsSetup=%d, Port=%s", 
                 cp->Type, cp->bEnabled, cp->b_IsSetup, cp->GetDSPort().c_str());
    ...
    if (cp->b_IsSetup) {
      ...
      wxLogMessage("    Skipping (already setup)");
      continue;
    }
    ...
    if (!cp->bEnabled) {
      wxLogMessage("    Skipping (disabled)");
      continue;
    }

    // Make any new or re-enabled drivers
    wxLogMessage("    Calling MakeCommDriver");
    MakeCommDriver(cp);
    cp->b_IsSetup = TRUE;
    enabled_conns.push_back(cp->GetStrippedDSPort());
    wxLogMessage("    Driver created and setup complete");
  }
  
  wxLogMessage("UpdateDatastreams: Complete");
}
