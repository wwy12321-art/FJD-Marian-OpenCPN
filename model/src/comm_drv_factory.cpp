...
void MakeCommDriver(const ConnectionParams* params) {
  ...
  wxLogMessage("MakeCommDriver: %s (Type=%d)", params->GetDSPort().c_str(), params->Type);
  ...
  switch (params->Type) {
    ...
    case DATAFILE: {
      wxLogMessage("MakeCommDriver: Creating File Replay driver (NO REGISTRATION)");
      wxLogMessage("  File: %s", params->FilePath.c_str());
      wxLogMessage("  Speed: %d", params->ReplaySpeed);
      wxLogMessage("  Loop: %d", params->ReplayLoop);
      
      try {
        auto driver = std::make_unique<FileReplayCommDriver>(
            params->FilePath.ToStdString(), params->ReplaySpeed, params->ReplayLoop);
        
        if (driver) {
          wxLogMessage("MakeCommDriver: Driver created, setting listener");
          driver->SetListener(listener);
          
          // WORKAROUND: Do NOT register driver to avoid evt_driverlist_change crash
          // Instead, just start the thread and let it send messages directly
          // The driver will be kept alive as a static object
          wxLogMessage("MakeCommDriver: Starting replay without driver registration");
          
          // Keep driver alive by storing as static
          static std::unique_ptr<FileReplayCommDriver> g_replay_driver;
          g_replay_driver = std::move(driver);
          
          // Start replay with delay
          wxLogMessage("MakeCommDriver: Starting replay thread (skipping registration)");
          g_replay_driver->SetInitialDelay(3000);  // 3 second delay before sending
          g_replay_driver->StartReplay();
          
          wxLogMessage("MakeCommDriver: File Replay driver started (NOT registered)");
        }
      } catch (const std::exception& e) {
        wxLogError("MakeCommDriver: Exception creating FileReplay driver: %s", e.what());
      }
      break;
    }
