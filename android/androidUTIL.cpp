 ...
 * Modified: 2025-10-30 - Added file replay support
 *
 ***************************************************************************
...
int androidApplySettingsString(wxString settings, ArrayOfCDI *pACDI) {
  wxLogMessage(_T("androidApplySettingsString: START"));
  wxLogMessage(_T("Settings string: %s"), settings.c_str());
  
  // Create marker file to verify code execution
  wxFile marker("/sdcard/Download/opencpn_debug.txt", wxFile::write);
  if (marker.IsOpened()) {
    marker.Write("androidApplySettingsString called\n");
    marker.Write(settings);
    marker.Close();
  }
...
// FIXME (dave)
  ...
  // Process File Replay Connection
  wxStringTokenizer tkFile(settings, _T(";"));
  bool bFileReplayEnabled = false;
  wxString fileReplayPath;
  int fileReplaySpeed = 1;
  bool fileReplayLoop = false;
  
  while (tkFile.HasMoreTokens()) {
    wxString token = tkFile.GetNextToken();
    wxString pref = token.BeforeFirst(':');
    wxString val = token.AfterFirst(':');
    
    if (pref.IsSameAs(_T("prefb_fileReplayEnable"))) {
      bFileReplayEnabled = val.IsSameAs(_T("1"));
      wxLogMessage(_T("File replay enabled: ") + val);
    } else if (pref.IsSameAs(_T("pref_fileReplayPath"))) {
      fileReplayPath = val;
      wxLogMessage(_T("File replay path: ") + val);
    } else if (pref.IsSameAs(_T("pref_fileReplaySpeed"))) {
      long speed;
      if (val.ToLong(&speed)) {
        fileReplaySpeed = speed;
      }
      wxLogMessage(_T("File replay speed: ") + val);
    } else if (pref.IsSameAs(_T("prefb_fileReplayLoop"))) {
      fileReplayLoop = val.IsSameAs(_T("1"));
      wxLogMessage(_T("File replay loop: ") + val);
    }
  }
  
  // Check if we need to create/update file replay connection
  if (bFileReplayEnabled && !fileReplayPath.IsEmpty()) {
    ConnectionParams *pFileReplayParams = NULL;
    
    // Check if connection already exists
    for (auto &xcp : TheConnectionParams()) {
      if (DATAFILE == xcp->Type) {
        pFileReplayParams = xcp;
        break;
      }
    }
    
    bool b_needCreate = false;
    if (pFileReplayParams) {
      // Update existing params
      if (pFileReplayParams->FilePath != fileReplayPath ||
          pFileReplayParams->ReplaySpeed != fileReplaySpeed ||
          pFileReplayParams->ReplayLoop != fileReplayLoop ||
          !pFileReplayParams->bEnabled) {
        
        wxLogMessage(_T("Updating file replay connection"));
        pFileReplayParams->FilePath = fileReplayPath;
        pFileReplayParams->ReplaySpeed = fileReplaySpeed;
        pFileReplayParams->ReplayLoop = fileReplayLoop;
        pFileReplayParams->bEnabled = true;
        pFileReplayParams->b_IsSetup = false;  // Force recreation
        rr |= NEED_NEW_OPTIONS;
      }
    } else {
      // Create new connection
      wxLogMessage(_T("Creating new file replay connection"));
      
      // Create ConnectionParams for file replay
      // Type(5=DATAFILE);NetProtocol;NetworkAddress;NetworkPort;Protocol;Port;Baudrate;ChecksumCheck;IOSelect;InputSentenceListType;InputSentenceList;OutputSentenceListType;OutputSentenceList;Priority;Garmin;GarminUpload;FurunoGP3X;bEnabled;UserComment;AutoSKDiscover;socketCAN_port;NoDataReconnect;DisableEcho;AuthToken;FilePath;ReplaySpeed;ReplayLoop
      wxString sFileReplay = wxString::Format(
          _T("5;0;;0;0;;4800;1;0;0;;0;;0;0;0;0;1;File Replay;0;;0;0;;%s;%d;%d"),
          fileReplayPath.c_str(), fileReplaySpeed, fileReplayLoop ? 1 : 0);
      
      wxLogMessage(_T("File replay connection string: ") + sFileReplay);
      
      ConnectionParams *new_params = new ConnectionParams(sFileReplay);
      new_params->bEnabled = true;
      TheConnectionParams().push_back(new_params);
      rr |= NEED_NEW_OPTIONS;
    }
  } else if (!bFileReplayEnabled) {
    // Disable file replay if it was enabled before
    for (auto &xcp : TheConnectionParams()) {
      if (DATAFILE == xcp->Type && xcp->bEnabled) {
        wxLogMessage(_T("Disabling file replay connection"));
        xcp->bEnabled = false;
        xcp->b_IsSetup = false;
        rr |= NEED_NEW_OPTIONS;
        break;
  ...
  // Trigger datastream update if connections changed
  wxLogMessage(_T("androidApplySettingsString: END, rr=0x%X"), rr);
  wxLogMessage(_T("androidApplySettingsString: NEED_NEW_OPTIONS=%d, has_flag=%d"), 
               NEED_NEW_OPTIONS, (rr & NEED_NEW_OPTIONS) ? 1 : 0);
  
  // Write debug info to file
  wxFile marker2("/sdcard/Download/opencpn_debug_end.txt", wxFile::write);
  if (marker2.IsOpened()) {
    wxString debug = wxString::Format("rr=0x%X, NEED_NEW_OPTIONS=0x%X, has_flag=%d\n", 
                                      rr, NEED_NEW_OPTIONS, (rr & NEED_NEW_OPTIONS) ? 1 : 0);
    marker2.Write(debug);
    marker2.Close();
  }
  
  if (rr & NEED_NEW_OPTIONS) {
    wxLogMessage(_T("File replay: Triggering UpdateDatastreams"));
    UpdateDatastreams();
  } else {
    wxLogMessage(_T("File replay: NOT triggering UpdateDatastreams (no flag)"));
  }
