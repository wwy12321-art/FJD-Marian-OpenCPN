  ...
  while (!dun) {
    ...
    switch (record.record_type) {
      ...
      case VECTOR_EDGE_NODE_TABLE_RECORD: {
        ...
        int nCount;
        memcpy(&nCount, pRun, sizeof(int));
        pRun += sizeof(int);

        for (int i = 0; i < nCount; i++) {
          int featureIndex;
          memcpy(&featureIndex, pRun, sizeof(int));
          pRun += sizeof(int);

          int pointCount;
          memcpy(&pointCount, pRun, sizeof(int));
          pRun += sizeof(int);
...
bool Osenc::CreateMultiPointFeatureGeometryRecord200(OGRFeature *pFeature,
  ...
  ps += 5;
  // Use memcpy to avoid unaligned access on ARM devices
  int nPoints;
  memcpy(&nPoints, ps, sizeof(int));  // point count
  ...
  for (int ip = 0; ip < nPoints; ip++) {
    ...
    // Use memcpy to safely write float values
    float east = easting;
    float north = northing;
    float deep = depth;
    
    memcpy(pd_run, &east, sizeof(float));
    pd_run += sizeof(float);
    memcpy(pd_run, &north, sizeof(float));
    pd_run += sizeof(float);
    memcpy(pd_run, &deep, sizeof(float));
    pd_run += sizeof(float);
...
bool Osenc::CreateLineFeatureGeometryRecord200(S57Reader *poReader,
  ...
  // Use memcpy to avoid unaligned access on ARM devices
  int ip;
  memcpy(&ip, ps + 5, sizeof(int));  // point count

  pd += 9;
  ps += 9;
  
  // Use byte pointers to avoid alignment issues
  unsigned char *ps_run = ps;
  unsigned char *pd_run = pd;
  ...
  for (int i = 0; i < ip; i++) {  // convert doubles to floats
    ...
    // Read lon and lat using memcpy to avoid unaligned access
    double east_d, north_d;
    memcpy(&east_d, ps_run, sizeof(double));
    ps_run += sizeof(double);
    memcpy(&north_d, ps_run, sizeof(double));
    ps_run += sizeof(double);
    ...
    // Write easting and northing as float using memcpy
    float easting_f = easting;
    float northing_f = northing;
    memcpy(pd_run, &easting_f, sizeof(float));
    pd_run += sizeof(float);
    memcpy(pd_run, &northing_f, sizeof(float));
    pd_run += sizeof(float);
...
void Osenc::CreateSENCVectorEdgeTableRecord200(Osenc_outstream *stream,
  ...
  while (NULL != pEdgeVectorRecordFeature) {
    ...
    if (nPoints) {
      ...
      int record_id = pEdgeVectorRecordFeature->GetFieldAsInteger("RCID");
      memcpy(pRun, &record_id, sizeof(int));
      pRun += sizeof(int);
      ...
      memcpy(pRun, &nPointReduced, sizeof(int));
      pRun += sizeof(int);
      ...
      if ((iField == 1) || (iField > 7)) {
        ...
        switch (OGRvalueType) {
          ...
          case 1:  // Integer list
          {
            ...
            for (int i = 0; i < nCount; i++) {
              memcpy(pBuffRun, &aValueList[i], sizeof(int));
              pBuffRun++;
            }
...
PolyTessGeo *Osenc::BuildPolyTessGeo(_OSENC_AreaGeometry_Record_Payload *record,
  ...
  for (int i = 0; i < nContours; i++) {
    int pointCount;
    memcpy(&pointCount, contour_pointcount_array_run, sizeof(int));
    *pctr++ = pointCount;
    contour_pointcount_array_run++;
  }
